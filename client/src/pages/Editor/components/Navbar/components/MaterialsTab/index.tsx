...
function computeMeshNames(viewer: Viewer): string[] {
...
export function MaterialsTab({ viewer, modelId }: MaterialsTabProps) {
  const [meshNames, setMeshNames] = useState<string[]>(() => computeMeshNames(viewer));
  const [prevViewer, setPrevViewer] = useState<Viewer>(viewer);
  const [expanded, setExpanded] = useState<Record<string, boolean>>({});
  const [forms, setForms] = useState<Record<string, MaterialOverrideUpsertDto>>({});

  const { data: materials = [], isFetching } = useGetMaterialsQuery({ modelId: modelId! }, { skip: !modelId });
  ...
  // Adjust mesh names when the viewer instance is swapped.
  if (prevViewer !== viewer) {
    setPrevViewer(viewer);
    setMeshNames(computeMeshNames(viewer));
  }

  // Subscribe to scene changes and refresh mesh names on each change. setState lives in the
  // event callback (allowed by react-hooks/set-state-in-effect), not synchronously in the body.
  useEffect(() => {
  ...
  // setState during render is supported for "adjust state on prop change".
  ...
  const toggleExpanded = (meshName: string) => setExpanded((prev) => ({ ...prev, [meshName]: !prev[meshName] }));
                  ...
                  <Collapse expanded={isOpen}>
                    ...
                    >
                      ...
                      <ColorInput
                        ...
                        onChange={(v) => setFormField(meshName, 'emissiveHex', v || undefined)}
                        eyeDropperIcon={<IconFocus2 style={{ width: rem(14), height: rem(14) }} />}
                        format="hex"
                        fixOnBlur
                      />
                      ...
                      <Switch
                        ...
                        onChange={(e) => setFormField(meshName, 'wireframe', e.target.checked)}
                      />
                      ...
                      {TEXTURE_SLOTS.map(({ key, label, urlKey }) => {
                        ...
                        return (
                          <Group key={key} justify="space-between" wrap="nowrap" gap="xs">
                            <Text size="xs" c="dimmed" style={{ flexShrink: 0, minWidth: 90 }}>
                              {label}
                            </Text>
                            <Group gap={4} wrap="nowrap">
                              {url && <Image src={url} w={24} h={24} fit="contain" radius="xs" />}
                              {url && (
                                <Tooltip label="Remove texture">
                                  <ActionIcon
                                    ...
                                    variant="subtle"
                                    color="red"
                                    onClick={() => handleTextureDelete(meshName, key)}
                                  >
                                    <IconX size={10} />
                                  </ActionIcon>
                                </Tooltip>
                              ...
                              <FileButton
                                onChange={(file) => handleTextureUpload(meshName, key, file)}
                                accept="image/png,image/jpeg,image/webp"
                              >
                                {(props) => (
                                  <Tooltip label={`Upload texture — ${TEXTURE_HINT}`}>
                                    <ActionIcon size="xs" variant="subtle" {...props}>
                                      <IconUpload size={10} />
                                    </ActionIcon>
                                  </Tooltip>
                                )}
                      ...
                      <Group gap="xs" justify="flex-end" mt={4}>
                        {override && (
                          <Button size="xs" variant="subtle" color="red" onClick={() => handleReset(meshName)}>
                            Reset
                          </Button>
                        )}
