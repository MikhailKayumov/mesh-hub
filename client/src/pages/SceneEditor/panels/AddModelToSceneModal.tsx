...
function ModelCard({ model, onAdd }: { model: Model3DResponseDto; onAdd: (modelId: string) => void }) {
    ...
    return (
        ...
        >
            ...
            <Box p="xs">
                <Text size="xs" fw={500} lineClamp={1}>{model.name}</Text>
                <Badge size="xs" variant="light" mt={2}>{model.file.extension.toUpperCase()}</Badge>
            </Box>
        </Box>
    );
}
...
export function AddModelToSceneModal({ opened, onClose, scene }: Props) {
    ...
    const { data: userModels, isLoading: isLoadingUser } = useCurrentUser3DModelsQuery(
        { size: 100 },
        { skip: !opened },
    );
