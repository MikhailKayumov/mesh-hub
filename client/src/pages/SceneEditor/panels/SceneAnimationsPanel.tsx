...
export function SceneAnimationsPanel({
    ...
    // Refresh animated IDs after scene finishes loading
    useEffect(() => {
        if (!isSceneLoading) {
            setAnimatedIds(getAnimatedObjectIds());
        }
    }, [isSceneLoading, getAnimatedObjectIds]);
    ...
    if (animatedObjects.length === 0) {
        return (
            <EmptyData
                message={isSceneLoading ? 'Loading scene…' : 'No animated objects in this scene'}
                p="md"
            />
        );
    }
