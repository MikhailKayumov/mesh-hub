...
export function SceneConfigPanel({ scene, updateBackgroundColor, updateAmbientLight, loadHdri }: Props) {
    ...
    const setFogField = <K extends keyof FogState>(key: K, value: FogState[K]) =>
        setFog((f) => ({ ...f, [key]: value }));
            ...
            {hasHdri ? (
                ...
                >
                    <Stack align="center" gap={4} py={4} style={{ pointerEvents: 'none' }}>
                        ...
                        <Text size="xs" ta="center">Drop .hdr file here or click to select (max 20 MB)</Text>
                    </Stack>
                </Dropzone>
            )}
