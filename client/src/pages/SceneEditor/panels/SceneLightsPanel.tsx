        ...
        <Paper p="xs" withBorder style={{ position: 'relative' }}>
            ...
            <Stack gap="xs">
                ...
                <Group gap="xs" justify="flex-end">
                    ...
                    <Button size="xs" onClick={() => onSave(form)}>Save</Button>
                </Group>
            </Stack>
        </Paper>
    );
...
export function SceneLightsPanel({ scene, selectedLightId, onSelectLight, syncLights }: Props) {
    ...
    const handleRemove = (light: SceneLightResponseDto) => {
        modals.openConfirmModal({
            title: 'Delete light',
            children: <Text size="sm">Delete this <b>{light.type}</b> light?</Text>,
            ...
            <Group justify="space-between" px={4} pt={4}>
                ...
                <Tooltip label="Add light">
                    <ActionIcon
                        size="sm"
                        variant="subtle"
                        onClick={() => { setAddOpen((o) => !o); setEditingId(null); }}
                    >
                        <IconPlus size={14} />
                    </ActionIcon>
                </Tooltip>
                    ...
                    {scene.lights.length === 0 && !addOpen ? (
                        ...
                        scene.lights.map((light) => (
                            <Stack key={light.id} gap={0}>
                                ...
                                >
                                    <Group justify="space-between" wrap="nowrap">
                                        <Group gap="xs" wrap="nowrap">
                                            ...
                                            <Text size="xs" tt="capitalize">{light.type}</Text>
                                            <Badge size="xs" variant="light">{light.intensity.toFixed(1)}</Badge>
                                        </Group>
                                ...
                                <Collapse in={editingId === light.id}>
                                    <Box mt={4}>
                                        <LightForm
