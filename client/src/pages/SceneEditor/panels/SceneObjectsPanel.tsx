...
export function SceneObjectsPanel({ scene, selectedObjectId, onSelectObject }: Props) {
  ...
  const handleRemove = (objectId: string, name: string) => {
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Remove <b>{name}</b> from the scene?
        </Text>
      ),
  ...
  return (
    <>
              ...
              scene.objects.map((obj) => (
                <UnstyledButton
                  ...
                  style={(theme) => ({
                    ...
                    backgroundColor: selectedObjectId === obj.id ? theme.colors.dark[5] : 'transparent',
                  })}
                >
      ...
      <AddModelToSceneModal opened={addModalOpen} onClose={() => setAddModalOpen(false)} scene={scene} />
    </>
  );
}
