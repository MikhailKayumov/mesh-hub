...
function SceneInfoCard({ scene }: { scene: SceneResponseDto }) {
    return (
        <Paper p="sm" withBorder>
            <Stack gap="xs">
                <Text fw={600} size="sm">{scene.name}</Text>
                <Group gap="xs">
                    <Badge size="sm" variant="outline" tt="capitalize">{scene.visibility}</Badge>
                </Group>
                <Divider />
                ...
                <Group justify="space-between">
                    <Text size="xs" c="dimmed">Lights</Text>
                    <Text size="xs">{scene.lights.length}</Text>
                </Group>
                {scene.updatedAt && (
                    <Group justify="space-between">
                        <Text size="xs" c="dimmed">Last saved</Text>
                        <Text size="xs">{new Date(scene.updatedAt).toLocaleString()}</Text>
                    </Group>
                )}
                <Text size="xs" c="dimmed" mt={4}>Select an object or light to edit its properties.</Text>
            </Stack>
        </Paper>
    );
}
...
export function ScenePropertiesPanel({ scene, selectionState, syncLights }: Props) {
    const selectedObject =
        selectionState?.type === 'object'
            ? scene.objects.find((o) => o.id === selectionState.id)
            : undefined;
    const selectedLight =
        selectionState?.type === 'light'
            ? scene.lights.find((l) => l.id === selectionState.id)
            : undefined;
