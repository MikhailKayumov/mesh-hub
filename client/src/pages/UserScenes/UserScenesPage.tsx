    ...
    <>
      <Card shadow="sm" radius="md" withBorder>
        ...
        <Stack gap={6} mt="sm">
          ...
          <Group gap={6}>
            <Badge size="xs" variant="light" color={VISIBILITY_COLOR[scene.visibility]}>
              {VISIBILITY_LABEL[scene.visibility]}
            </Badge>
          </Group>

          <Text size="xs" c="dimmed">
            {scene.objectCount} объект
            {scene.objectCount === 1 ? '' : scene.objectCount >= 2 && scene.objectCount <= 4 ? 'а' : 'ов'} · Изменено{' '}
