...
import { IconVolume, IconVolumeOff, IconPlayerPlay, IconPlayerStop, IconRepeat, IconRepeatOff } from '@tabler/icons-react';
...
interface AudioToolbarProps {
    modelId: string;
    className?: string;
    viewer?: { playAudio: (url: string, opts?: { loop?: boolean; volume?: number }) => ThreeAudio; stopAllAudio: () => void; audioContext: AudioContext | null };
    modelSrc?: string;
}
...
export function AudioToolbar({ modelId, className, viewer }: AudioToolbarProps) {
    ...
    return (
        <Group className={className} gap={4} wrap="nowrap">
            <Text size="xs" c="dimmed" mr={4}>Audio</Text>
            <Select
                size="xs"
                data={selectData}
                value={selectedId}
