...
function UploadVersionModal({ modelId, opened, onClose }: { modelId: string; opened: boolean; onClose: () => void }) {
...
function VersionCard({
  ...
  const uploaderName =
    [version.uploader.firstName, version.uploader.lastName].filter(Boolean).join(' ') || 'Пользователь';
            ...
            {canEdit && !version.isActive && (
              <>
                <Tooltip label="Сделать активной">
                  <ActionIcon variant="subtle" color="green" onClick={handleActivate} loading={activating} size="sm">
                    <IconCheck size={16} />
                  </ActionIcon>
                </Tooltip>
                <Tooltip label="Удалить версию">
                  <ActionIcon variant="subtle" color="red" onClick={openDelete} loading={deleting} size="sm">
                    <IconTrash size={16} />
                  </ActionIcon>
                </Tooltip>
              </>
...
export function VersionHistory({ modelId, canEdit, viewer }: VersionHistoryProps) {
  ...
  return (
    <Stack gap={12}>
      {canEdit && (
        <Button leftSection={<IconCloudUpload size={16} />} variant="light" onClick={openUpload} fullWidth>
      ...
      {!versions?.length ? (
        ...
        versions.map((v) => <VersionCard key={v.id} version={v} modelId={modelId} canEdit={canEdit} viewer={viewer} />)
      )}

      {canEdit && <UploadVersionModal modelId={modelId} opened={uploadOpened} onClose={closeUpload} />}
    </Stack>
  );
}
